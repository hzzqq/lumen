...
// 校验十二步接线全链路 + presetToParams 字段数不少于 125
const fs = require('fs');
...
// presetToParams 实际执行返回不少于 125 字段(ci461 后为 131，字段完整性由 _ci461_preset_fields_test 交叉审计)，5 个新效果默认 0 / 透传
{
  const m = main.match(/function presetToParams\(p\)\{[\s\S]*?\n\}/);
  const f = eval('(' + m[0] + ')');
  const s = f({});
  ok('presetToParams 返回不少于 125 字段', Object.keys(s).length >= 125);
