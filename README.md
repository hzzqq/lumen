...
- **体积雾颜色（fogColor）**：雾的散射颜色由固定值改为可调 `uFogColor` uniform，UI 取色器实时调色，URL 参数 / 预设 / 序列化 / 导入导出全链路打通（ci105）。
- **视场角 FOV 可调**：新增 `uFov` uniform 控制主射线锥角（默认 50°），UI 滑块 `20°~90°` 实时调节视野宽窄，URL 参数 / 预设 / 序列化 / 导入导出全链路打通（ci109）。
- **背景渐变天穹（bgGradient）**：新增 `bgTop` / `bgBottom` 双色顶/底渐变天穹，与程序化太阳天空可切换，UI 取色器分别调顶/底色，URL 参数 / 预设 / 序列化 / 导入导出全链路打通（ci113）。
- **调试可视化（debug）**：新增 `uDebug` 模式（1=反照率 / 2=法线 / 3=深度），实时把内部 G-buffer 直出到屏幕，便于排查材质 / 几何 / 光照问题（ci117）。
- **萤火虫钳制（firefly clamp）**：新增 `uClamp` 高光钳制上限，路径追踪中对累积辐射 `L = min(L, uClamp)` 抑制单路径超高亮噪点（萤火虫），UI 滑块 `0~20`（0 = 关闭），默认关闭（ci121）。
...
# _fogcolor_test.js (25/25) 体积雾颜色：uFogColor 接线 / 序列化 / 有界
# _fov_test.js      (16/16) 视场角：uFov 锥角 / 滑块范围 / 序列化
